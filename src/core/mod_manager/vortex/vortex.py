...
class Vortex(ModManager[ProfileInfo]):
    ...
    def load_instance(
        ...
        update(
            update_callback,
            ProgressUpdate(
                status_text=self.tr("Loading profile {0}...").format(instance_name)
            ),
        )
    ...
    def _load_mods(
        ...
        update(
            update_callback,
            ProgressUpdate(
                status_text=self.tr("Loading mods from profile {0}...").format(
                    instance_name
                )
            ),
        )
        ...
        for m, modname in enumerate(modnames):
            ...
            update(
                update_callback,
                ProgressUpdate(
                    status_text=self.tr("Loading mods from profile {0}: {1}...").format(
                        instance_name, modname
                    ),
                    value=m,
    ...
    @override
    def prepare_instance(self, instance_data: ProfileInfo) -> None:
    ...
    def install_mod(
        ...
        if not db_mod_data:
            ...
            dl_game_id: str
            try:
                dl_game_id = GameService.get_game_by_nexus_id(
                    mod.metadata.game_id
                ).id.lower()
            except ValueError:
                self.log.warning(
    ...
    def finalize_instance(
        ...
        profile_data["features"]["local_saves"] = instance.separate_save_games
        ...
        if activate_instance:
            ...
            self.__level_db.set_key(key, instance_data.id)

            # Set last active profile
            key = "settings###profiles###lastActiveProfile###"
            key += instance_data.game.id.lower()
            self.__level_db.set_key(key, instance_data.id)
