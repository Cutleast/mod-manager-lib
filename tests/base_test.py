...
class BaseTest(CoreBaseTest):
    ...
    def game_service(self, data_folder: Path) -> Generator[GameService, None, None]:
        """
        Fixture to initialize and provide the GameService for tests.
        """

        game_data: str = (data_folder / "games.json").read_text("utf8")

        yield GameService(game_data)
    ...
    @pytest.fixture
    def instance(
        self, mo2_instance_info: MO2InstanceInfo, test_fs: FakeFilesystem
    ) -> Instance:
        """
        ...
        return ModOrganizer().load_instance(mo2_instance_info, 100)
